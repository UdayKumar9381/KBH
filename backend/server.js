...
const SPREADSHEET_ID = process.env.SPREADSHEET_ID || "1frMdQU6mBUtFZbyIkEsw2shUll6U9xWD5hYTcHGz0I8";
...
async function initializeAuth() {
  try {
    ...
    if (process.env.GOOGLE_CREDENTIALS) {
      ...
      const keyFilePath = path.join(__dirname, "config", "kbhfoodreport-ae688541f8a0.json");
...
function resolveSheetName(sheet) {
  ...
  if (!VALID_SHEETS.includes(sheetName)) {
    throw new Error(
      `Invalid sheet name '${sheet}'. Must be between JANUARY 2022 and DECEMBER 2030.`
    );
  }
...
async function getSheetData(sheetName) {
  try {
    ...
    if (!values || values.length === 0) {
      console.warn(`⚠️ No data at all found in '${sheetName}' (values is empty)`);
      throw new Error(`No data found in '${sheetName}'`);
    }

    if (values.length < 1) {
      console.warn(`⚠️ Sheet '${sheetName}' has no rows (length < 1)`);
      throw new Error(`Sheet '${sheetName}' is empty`);
    }

    const headers = values[0].map(h => (h || "").trim().toUpperCase());
    console.log(`📊 Headers found in '${sheetName}':`, JSON.stringify(headers));

    if (values.length === 1) {
      console.warn(`⚠️ Sheet '${sheetName}' only contains headers, no data rows.`);
    }
    ...
    for (const row of dataRows) {
      ...
      const record = {
        ...
        AMOUNT: amount,
        YEAR: getVal(row, ["YEAR", "ACADEMIC YEAR"]) || new Date().getFullYear().toString(),
      };
    ...
    if (cleaned.length === 0 && dataRows.length > 0) {
      console.warn(`⚠️ Warning: ${dataRows.length} rows were present but 0 were kept after filtering. Check header keys!`);
      console.log(`🔍 Sample row 1 content:`, JSON.stringify(dataRows[0]));
    }

    // Save to cache
    cache.set(sheetName, cleaned);
...
app.get("/sheets", async (req, res) => {
  try {
    ...
    // Check cache first
    const cachedSheets = cache.get("AVAILABLE_SHEETS_LIST");
    if (cachedSheets) {
    ...
    const responseData = {
      ...
      valid_range: "JANUARY 2022 - DECEMBER 2030",
      total_sheets: availableSheets.length,
    };

    // Cache the sheets list
    cache.set("AVAILABLE_SHEETS_LIST", responseData);

    res.json(responseData);
  } catch (error) {
    console.error("❌ Error in /sheets:", error);
    res.status(500).json({ error: `Metadata error: ${error.message}`, details: error.stack });
  }
});
...
app.get("/summary", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: `Summary error: ${error.message}`, details: error.stack });
  }
});
...
app.get("/roomwise", async (req, res) => {
  try {
    ...
    console.error("❌ Roomwise error:", error);
    res.status(500).json({ error: `Roomwise error: ${error.message}`, details: error.stack });
  }
});
...
app.get("/yearwise", async (req, res) => {
  try {
    ...
    console.error("❌ Yearwise error:", error);
    res.status(500).json({ error: `Yearwise error: ${error.message}`, details: error.stack });
  }
});
...
(async () => {
  try {
    ...
    console.log("🔍 Auth initialized, starting Express app...");
